...
export default function AdminLogin() {
    ...
    const onSubmit = async (e: FormEvent) => {
        e.preventDefault()
        try {
            const { token, user } = await authLogin({ email, password, role: 'ADMIN' })
            setAuthToken(token)
            login({ role: 'ADMIN', email: user.email, name: user.name, token })
            pushToast({ title: 'Admin logged in', type: 'success' })
