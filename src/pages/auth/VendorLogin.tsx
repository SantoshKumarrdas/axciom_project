...
export default function VendorLogin() {
    ...
    const onSubmit = async (e: FormEvent) => {
        e.preventDefault()
        try {
            const { token, user } = await authLogin({ email, password, role: 'VENDOR' })
            setAuthToken(token)
            login({ role: 'VENDOR', email: user.email, name: user.name, token })
            pushToast({ title: 'Vendor logged in', type: 'success' })
